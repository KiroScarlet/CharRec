        ...
        with tf.variable_scope('cnn'):
            ...
            for i in range(FLAGS.cnn_count):
                with tf.variable_scope('unit-%d' % (i + 1)):
                    x = self._conv2d(x, 'cnn-%d' % (i + 1), 3, filters[i], filters[i + 1], strides[0]) # _conv2d(self, x, name, filter_size, in_channels, out_channels, strides)
                    #x = self._batch_norm('bn%d' % (i + 1), x)
                    x = self._leaky_relu(x, FLAGS.leakiness)
                    x = self._max_pool(x, 2, strides[1]) # _max_pool(self, x, ksize, strides)
